...
function M.run(config)
	...
	if frt.request_data and frt.request_data.headers then
		frt.request_data.headers = redact.redact_headers(frt.request_data.headers, config, "request")
	end

	if frt.response_data and frt.response_data.headers then
		frt.response_data.headers = redact.redact_headers(frt.response_data.headers, config, "response")
	end
